# Requirements Archive: v4.0 Agent Documentation Layer
...
- [x] **BIND-01**: User can configure a doc repo URL, branch, project directory slug, and access token per profile (`docs_repo`, `docs_branch`, `docs_project_dir`, `DOCS_REPO_TOKEN` in profile.json / .env)
- [x] **BIND-02**: `DOCS_REPO_TOKEN` is stored in the host-only profile `.env` and is never mounted into the Claude container
- [x] **BIND-03**: Profiles with legacy `report_repo` / `REPORT_REPO_TOKEN` (Phase 16) continue to work without migration — new fields are aliases
...
| Requirement | Phase | Status |
|-------------|-------|--------|
| BIND-01 | Phase 23 | Complete |
| BIND-02 | Phase 23 | Complete |
| BIND-03 | Phase 23 | Complete |
