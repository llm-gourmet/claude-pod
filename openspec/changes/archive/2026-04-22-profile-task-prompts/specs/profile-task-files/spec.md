...
1. `~/.claude-secure/profiles/<name>/tasks/<event_type>.md` — event-specific task
2. `~/.claude-secure/profiles/<name>/tasks/default.md` — profile default task
...
When a new profile is created via `claude-secure profile <name> create`, the command SHALL create a `tasks/` subdirectory containing a `default.md` placeholder file.

#### Scenario: New profile has tasks/default.md
- **WHEN** `claude-secure profile my-agent create` is run
- **THEN** `~/.claude-secure/profiles/my-agent/tasks/default.md` SHALL exist
- **AND** it SHALL contain a minimal placeholder task prompt

### Requirement: Global webhook templates removed
The directories `/opt/claude-secure/webhook/templates/` and `/opt/claude-secure/webhook/report-templates/` SHALL NOT be created by the installer and SHALL NOT be referenced by the spawn logic.

#### Scenario: Installer does not create global template directories
- **WHEN** `install.sh` is run
- **THEN** `/opt/claude-secure/webhook/templates/` SHALL NOT exist

#### Scenario: Spawn does not fall back to global templates
- **WHEN** no task file exists in the profile directory
- **THEN** spawn SHALL fail, not fall back to any path under `/opt/claude-secure/`
