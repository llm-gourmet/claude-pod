...
- **WHEN** `claude-pod gh-webhook-listener --add-connection --name myrepo --repo org/repo --webhook-secret shsec_xxx` is run
...
- **WHEN** `claude-pod webhook-listener --remove-connection myrepo` is run and `myrepo` exists
- **THEN** `connections.json` no longer contains a connection named `myrepo`

#### Scenario: Unknown connection name is rejected
- **WHEN** `claude-pod webhook-listener --remove-connection nonexistent` is run
...
- **WHEN** `claude-pod webhook-listener --list-connections` is run with two connections configured
...
`webhook/listener.py` SHALL read `~/.claude-pod/webhooks/connections.json` to find the connection matching an incoming webhook's repository full name. Profile directories SHALL NOT be scanned for webhook credentials.
...
### Requirement: listener stubs spawn — no claude-pod spawn call
**REMOVED** — replaced by `webhook-spawn-always` spec. `_spawn_worker` now calls `claude-pod spawn` unconditionally after HMAC and repo lookup. No action required; the listener upgrade replaces the stub automatically.

## REMOVED Requirements

### Requirement: listener stubs spawn — no claude-pod spawn call
**Reason**: Placeholder behaviour removed. `_spawn_worker` now calls `claude-pod spawn` unconditionally after HMAC and repo lookup (see `webhook-spawn-always` spec).
