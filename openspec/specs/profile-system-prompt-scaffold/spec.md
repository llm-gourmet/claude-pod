...
2. Instructs Claude to tell the user on the first response that the system prompt is a placeholder and can be updated on the host at `~/.claude-secure/profiles/<profile-name>/system_prompts/`.
...
- **THEN** `~/.claude-secure/profiles/myproj/system_prompts/default.md` exists
- **AND** the file contains the literal text `~/.claude-secure/profiles/myproj/system_prompts/`
