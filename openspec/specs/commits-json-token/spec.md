...
The `render_template` function in `bin/claude-pod` SHALL substitute the token `{{COMMITS_JSON}}` with the compact JSON representation of the `commits` array from the GitHub push event payload. If the `commits` key is absent, the token SHALL be replaced with the empty JSON array `[]`.
